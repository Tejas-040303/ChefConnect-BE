...
function CustomerReview({ data }) {
  ...
  const nextSlide = () => {
    setStartIndex((prevIndex) =>
      prevIndex + 1 >= safeData.length ? 0 : prevIndex + 1
    );
  };

  const prevSlide = () => {
    setStartIndex((prevIndex) =>
      prevIndex - 1 < 0 ? safeData.length - 1 : prevIndex - 1
    );
  };
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="relative">
          ...
          <div className="overflow-hidden">
            {safeData.length === 0 ? (
              <p className="text-center text-orange-500">No reviews available</p>
            ) : (
