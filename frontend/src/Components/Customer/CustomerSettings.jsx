...
function CustomerSettings() {
  ...
  const handlePasswordChange = async (e) => {
    ...
    try {
      ...
      await axios.patch('http://localhost:8080/customer/settings/password', passwordData, {
  ...
  const handleDeleteRequest = async () => {
    try {
      const token = localStorage.getItem('token');
      await axios.post('http://localhost:8080/customer/settings/delete-request', {}, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const handleComplaintSubmit = async (e) => {
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.post('http://localhost:8080/customer/settings/complaint', { complaint }, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const handleContactAdmin = async (e) => {
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.post('http://localhost:8080/customer/settings/contact-admin', { message: contactMessage }, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  return (
    <div className="text-amber-500 p-8 mx-8 mb-16">
